...
int main() {
    // Initialize the DataProcessing system
    DataProcessing dp;
    initDataProcessing(&dp);

    // Start connection (for Blade Runner ID 18 or 19)
    connectionStartBR(18);

    // Simulate receiving a command and executing it
    char buffer[BUFFER_SIZE];
    receivePacket(buffer);

    executeCommand(&dp, buffer);
